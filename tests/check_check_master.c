...
START_TEST(test_check_failure_msgs)
{
  ...
  for (i = 0; i < sub_ntests; i++) {
    ...
    if (not_equal) {
      ...
      char _emsg[MAXSTR];
      snprintf(_emsg, MAXSTR - 1,"For test %d:%s:%s Expected%s '%s', got '%s'",
       ...
       * in ck_abort_msg().
       */
      char *emsg = escape_percent(_emsg, MAXSTR);
...
START_TEST(test_check_all_msgs)
{
  ...
  if (not_equal) {
    ...
    char _emsg[MAXSTR];
    snprintf(_emsg, MAXSTR - 1, "For test %i:%s:%s expected%s '%s', got '%s'",
